...
class Zone extends \Frontend\Zone
{
    ...
    function getElements($languageId = null, $parentElementId = null, $startFrom = 0, $limit = null, $includeHidden = false, $reverseOrder = false)
    {
        ...
        if ($reverseOrder) {
            $dbElements = $this->db->getElements(
                $this->getName(),
                $parentElementId,
                $languageId,
                $this->currentElement ? $this->currentElement->getId() : null,
                $selectedId,
                'desc',
                $startFrom,
                $limit,
        ...
        foreach ($elements as $key => $element) { //link generation optimization.
            if ($elements[$key]->getType() == 'default') {
                $elements[$key]->setLink(
                    $site->generateUrl(
                        $languageId,
                        $this->getName(),
                        array_merge($urlVars, array($element->getUrl())),
                        null
                    )
                );
            }
    ...
    function getFirstElement($parentId, $level)
    {
        ...
        $elements = $this->db->getElements(
            $this->getName(),
            $parentId,
            $site->currentLanguage['id'],
            null,
            null,
            'asc',
            0,
